...
class Environment:
    ...
    def _cost_definition(self):
        ...
        cost_task_resources = np.random.normal(
            loc=0.5, scale=self.cost_std_dev, size=(self.num_resources, self.get_num_tasks())
        )
    ...
    def increase_capacity(self, filled_resources):
        ...
        remaining_resources = list(
            set(range(len(self.resource_capacities))) - set(filled_resources)
        )
        ...
        self.cost_task_resource = np.hstack(
            [
                self.cost_task_resource,
                np.random.normal(loc=0.5, scale=self.cost_std_dev, size=(self.num_resources, 1)),
            ]
        )
